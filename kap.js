...
program.name("kap").version(packageData.version);

program
  .command("uninstall <command-name>")
  .alias("rm")
  .description(
    "Will remove the command specified from the list of available command"
  )
  .action(
...
program
  ...
  .description(
    'Links current working directory as a command. If no command name is specified it will use the "command" property from package.json'
  )
  .alias("ln")
  .action(
    makeCommand((commandName) => {
